package com.normation.syncrepl.apacheds;
...
/*
 * A copy of ApacheDS SyncReplConsumer, just to have
 * it on hand for development. 
 * 
 * It's a v1.5.7 modified to work with ldap-share 0.9.19,
 * as it seems that ApacheDS 1.5.17 used an older version -
 * but I don't know which one exactly.
 *
 */
public class InitialApacheDSSyncReplConsumer {
    ...
    /** the logger */
    private static final Logger LOG = LoggerFactory.getLogger( InitialApacheDSSyncReplConsumer.class.getSimpleName() );
    ...
    private void _deleteEntries_( List<byte[]> limitedUuidList ) throws Exception
    {
        ...
        if( size == 1 )
        {
            ...
            filter = new EqualityNode<String>( SchemaConstants.ENTRY_UUID_AT, new StringValue( uuid )  );
        }
        else
        {
            filter = new OrNode();
            for ( int i=0; i < size; i++ )
            {
                ...
                EqualityNode<String> uuidEqNode = new EqualityNode<String>( SchemaConstants.ENTRY_UUID_AT, new StringValue( uuid ) );
